...
/**
 * Servlet implementation class RemoveTimeAction
 */
public class RemoveTimeAction extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public RemoveTimeAction() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (user == null) {
			String title = "Remove Time Failed";
		...
		if (poll.getCreator() != user.getId()) {
			String title = "Remove Time Failed";
			String message = "You are not the right person to remove time in this poll! Please login as the poll creater.";
			String button  = "Login";
			String url     = "login.jsp";
			request.setAttribute("title", title);
			request.setAttribute("message", message);
		...
		boolean result = db.removeTimeFromPoll(poll, time);
		
		if (!result) {
			String title = "Remove Time Failed";
			String message = "No such time found.";
			String button  = "Back";
